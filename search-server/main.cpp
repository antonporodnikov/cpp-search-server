...
public:
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status,
        ...
        if (document_id < 0 || find(ids_.begin(), ids_.end(), document_id) != ids_.end()) {
            throw invalid_argument("Некорретный ID"s);
        }
    ...
    int GetDocumentId(int index) const {
        if (index < 0 || (index + 1) > static_cast<int>(ids_.size())) {
...
private:
    ...
    static bool IsValidMinus(const string& minus_word) {
        // A valid minus-word not contain two minuses and single minus char
        if (minus_word[0] == '-' && minus_word[1] == '-') {
            return false;
        }
        if (minus_word == "-"s) {
            return false;
